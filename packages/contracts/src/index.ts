...
export const FoodSchema = z.object({
  ...
  /** For a special dish, the character whose specialty it is. */
  specialtyName: z.string().default(""),
  /** For a special dish, the normal dish it is an upgraded version of. */
  baseDishName: z.string().default(""),
});
...
/** A substat line: `rollValues` are the individual rolls (base + upgrades); `value` is their sum. */
export const ArtifactSubStatSchema = z.object({
  key: StatKeySchema,
  value: z.number(),
  rollValues: z.array(z.number()).default([]),
});
